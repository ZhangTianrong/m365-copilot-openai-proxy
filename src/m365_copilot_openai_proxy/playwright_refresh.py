        ...
        const result = await service.handleRequest(
            {
                ...
                tokenParams: {
                    clientId,
                    scope: "https://substrate.office.com/search/.default",
...
async def _capture_search_access_token(page) -> dict[str, object] | None:
    try:
        result = await page.evaluate(_SEARCH_TOKEN_JS)
    except Exception:
        return None
    if not isinstance(result, dict):
        return None
        ...
        while time.time() < deadline:
            ...
            if snapshot is not None:
                ...
                if search_auth:
                    ...
                    snapshot = current_auth_snapshot() or snapshot
                if settings.account_mode == "personal" and not auth_state.saw_personal_login_host:
                    raise PlaywrightRefreshError(
                        "M365_ACCOUNT_MODE is personal, but the browser session completed "
                        "without entering the personal Microsoft sign-in flow."
                    )
...
async def login_with_playwright(
    ...
    *,
    ...
    try:
        ...
        return write_auth_session(settings, auth_session)
    finally:
        if context is not None:
            await context.close()
        if manager is not None:
